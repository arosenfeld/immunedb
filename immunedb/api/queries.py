...
def _subject_to_dict(subject):
    return {
        'study': {'id': subject.study.id, 'name': subject.study.name},
...
def _clone_to_dict(clone):
    d = _fields_to_dict(
        [
            ...
            'overall_total_cnt_with_subclones',
        ],
        clone,
    )
...
def get_clones(
    session, filters, order_field, order_dir, subject_limit=None, paging=None
):
    ...
    for c in _page_query(clone_q, paging):
        ...
        query = (
            session.query(CloneStats.unique_cnt, CloneStats.total_cnt, Sample)
            .join(Sample)
            .filter(
                CloneStats.clone_id == c.id, ~CloneStats.sample_id.is_(None)
            )
            .order_by(
        ...
        for stat in query:
            stats_comb.append(
                {
                    'sample': {'id': stat.Sample.id, 'name': stat.Sample.name},
...
def get_clone_mutations(
    session, clone_id, threshold_type, threshold_val, sample_id=None
):
    clone_stats = (
        session.query(CloneStats.mutations, CloneStats.unique_cnt)
        .filter(
            CloneStats.clone_id == clone_id, CloneStats.sample_id == sample_id
        )
        .first()
    )
...
def get_clone_sequences(session, clone_id, get_collapse, paging):
    query = (
        ...
        .order_by(desc(SequenceCollapse.copy_number_in_subject))
    )
    ...
    if get_collapse:
        q = (
            session.query(Sequence, SequenceCollapse)
            .outerjoin(SequenceCollapse)
            .filter(
                Sequence.clone_id == clone_id,
                SequenceCollapse.copy_number_in_subject == 0,
            )
        ...
        for seq, collapse in q:
            key = (
                collapse.collapse_to_subject_sample_id,
...
def get_selection_pressure(session, clone_id):
    query = session.query(SelectionPressure).filter(
    ...
    for row in query:
        if row.sample_id is None:
            ...
            name = (
                session.query(Sample.name)
                .filter(Sample.id == row.sample_id)
                .first()
                .name
            )
        if name not in pressure:
            pressure[name] = {
                'pressure': {},
                'sample': {'id': row.sample_id, 'name': name},
            }
...
def get_clone_overlap(
    session, sample_ids, filter_type, order_by='total_cnt', paging=None
):
    ...
    for clone_id, functional, unique_cnt, total_cnt in stats:
        aggregated_stats.setdefault(clone_id, Counter()).update(
            {'unique_cnt': unique_cnt, 'total_cnt': total_cnt}
        )
    aggregated_stats = sorted(
        aggregated_stats.items(), key=lambda kv: kv[1][order_by], reverse=True
    )
    ...
    for clone_id, counts in aggregated_stats:
        ...
        query = (
            ...
            .order_by(desc(getattr(CloneStats, order_by)))
        )
        for stat in query:
            data = {
                'id': stat.sample_id,
...
def get_clones_in_samples(session, samples):
    return [
        e.id
        for e in session.query(distinct(Sequence.clone_id).label('id')).filter(
...
def get_clones_in_subject(session, subject_id):
    return [
        e.id
        for e in session.query(Clone).filter(Clone.subject_id == subject_id)
    ]
...
def get_v_usage(
    ...
    totals = {}
    prefix = ''
    for s in session.query(SampleStats).filter(
        SampleStats.filter_type == filter_type,
...
def get_all_subjects(session, paging=None):
    ...
    for subject in _page_query(
        ...
        total_seqs = (
            session.query(func.sum(SampleStats.sequence_cnt))
            .filter(
        ...
        if total_seqs is not None:
            info['total_samples'] = (
                ...
                .filter(Sample.subject == subject)
                .scalar()
            )
            info['total_seqs'] = int(total_seqs)
            info['unique_seqs'] = int(unique_seqs)
            info['total_clones'] = (
                session.query(func.count(Clone.id))
                .filter(Clone.subject_id == subject.id)
                .scalar()
            )
...
def get_subject(session, sid):
    ...
    samples = get_samples(
        ...
        [
            e.id
            for e in session.query(Sample.id).filter(Sample.subject_id == sid)
        ],
    )
...
def trace_seq_collapses(session, seq):
    collapse_info = (
        session.query(SequenceCollapse)
        .filter(
    ...
    return {
        ...
        'copy_number': collapse_info.collapse_to_seq.collapse.copy_number_in_subject,
        'instances': collapse_info.collapse_to_seq.collapse.instances_in_subject,
    }
...
def get_sequence(session, sample_id, seq_id):
    seq = (
        ...
        .filter(Sequence.sample_id == sample_id, Sequence.seq_id == seq_id)
        .one()
    )
    ret = _fields_to_dict(
        [
    ...
    ret['read_start'] = re.compile(r'[N-]*').match(seq.sequence).span()[1] or 0
...
def get_sequences(
    session, filters, order_field, order_dir, subject_id=None, paging=None
):
    """Gets a list of all clones"""
    res = []
