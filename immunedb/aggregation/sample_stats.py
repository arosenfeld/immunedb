...
_seq_contexts = {
    'all': {'record_filter': lambda seq: True, 'use_copy': True},
    'functional': {
    ...
    'unique': {'record_filter': lambda seq: seq.functional, 'use_copy': False},
    'unique_multiple': {
...
_clone_contexts = {
    ...
    'clones_functional': {'record_filter': lambda functional: functional},
...
class SampleStatsWorker(concurrent.Worker):
    ...
    def do_task(self, args):
        ...
        self.info(
            (
                'Processing {} for sample {}, include_outliers {}, '
                'only_full_reads {}'
            ).format(
    ...
    def _add_stat(self, stats, sample_id, include_outliers, only_full_reads):
        for name, stat in stats.items():
            ss = SampleStats(
                ...
                no_result_cnt=self._session.query(NoResult)
                .filter(NoResult.sample_id == sample_id)
                .count(),
            )
    ...
    def _calculate_seq_stats(
        self, sample_id, min_cdr3, max_cdr3, include_outliers, only_full_reads
    ):
        seq_statistics = {}
        ...
        query = self._session.query(
            ...
            (func.ceil(100 * Sequence.v_match / Sequence.v_length)).label(
                'v_identity'
            ),
        ...
        if not include_outliers and min_cdr3 is not None:
            query = query.filter(
                Sequence.cdr3_num_nts >= min_cdr3,
        ...
        self._add_stat(
            seq_statistics, sample_id, include_outliers, only_full_reads
        )

    def _calculate_clone_stats(
        self, sample_id, min_cdr3, max_cdr3, include_outliers, only_full_reads
    ):
        clone_statistics = {}
        ...
        query = (
            ...
            .filter(
                Sequence.sample_id == sample_id, ~Sequence.clone_id.is_(None)
            )
        )
        ...
        for clone in query:
            clone_info = (
                self._session.query(Clone.cdr3_nt)
                .filter(Clone.id == clone.clone_id)
                .first()
            )
        ...
        self._add_stat(
            clone_statistics, sample_id, include_outliers, only_full_reads
        )
...
def _queue_tasks(session, sample_id, force, tasks):
    logger.info(
        'Creating task queue to generate stats for sample {}.'.format(sample_id)
    )
    ...
    if existing_seq.first() is None and existing_nores.first() is None:
        logger.warning(
            '\tSKIPPING since there are no sequences in the ' 'sample'
        )
        return

    existing = (
        session.query(SampleStats.sample_id)
        .filter(SampleStats.sample_id == sample_id)
        .first()
        is not None
    )
    if force and existing:
        ...
        logger.warning(
            '\tSKIPPING stats since they already exists and the '
...
def run_sample_stats(session, args):
    np.seterr(all='raise')
    mod_log.make_mod(
        'sample_stats', session=session, commit=True, info=vars(args)
    )
