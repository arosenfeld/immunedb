...
def align_reference(path, index, sequences, nproc):
    cmd = (
        'bowtie2 --local -x {} -U {} -f --no-unal --no-sq --no-head '
        '--very-sensitive-local -p {}'
    ).format(index, sequences, nproc)
    proc = subprocess.Popen(
        shlex.split(cmd),
...
def create_seqs(read_seq, ref_seq, cigar, ref_offset, min_size, **kwargs):
    ...
    if len(final_ref) < min_size:
        ...
        final_ref = ''.join([final_ref, full_ref[ref_offset:min_size]])
        diff = len(final_ref) - len(final_seq)
        if diff > 0:
            seq_offset = len(final_seq) - final_seq.count('-')
            final_seq = ''.join(
                [final_seq, full_seq[seq_offset : seq_offset + diff]]
            )
            if len(skips) > 0:
...
def add_imgt_gaps(imgt_germline, aligned_germline, sequence, seq_start):
    for pos, size in gap_positions(imgt_germline):
        ...
        sequence = ''.join(
            (sequence[:pos], GAP_PLACEHOLDER * size, sequence[pos:])
        )
...
def process_sample(
    session, sample, indexes, temp, v_germlines, j_germlines, nproc
):
    indels = (
        session.query(
            Sequence.ai, Sequence.seq_id, Sequence.sample_id, Sequence.sequence
        )
        .filter(
    ...
    noresults = (
        session.query(NoResult)
        .filter(NoResult.sample_id == sample.id)
        .order_by(NoResult.seq_id)
    )
    ...
    if indels.count() == 0 and noresults.count() == 0:
        ...
        return
    logger.info(
        'Sample {} has {} indels and {} noresults'.format(
    ...
    if bucket not in indexes:
        ...
        logger.info(
            'Creating index for V-ties at {} length, {} '
    ...
    for line in get_reader(
        align_reference(temp, f'v_genes_{bucket}', seq_path, nproc)
    ):
        ...
        ref, seq, seq_start = add_imgt_gaps(
            sample_v_germlines[ref_gene], ref, seq, line['ref_offset']
        )
    ...
    for line in get_reader(
        align_reference(temp, f'j_genes_{bucket}', seq_path, nproc)
    ):
        line['ref_offset'] = int(line['ref_offset']) - 1
        ref_gene = line['reference']
        ref, seq, rem_seqs = create_seqs(
        ...
        full_seq = (
            alignments[line['seq_id']]['v_sequence']
        ...
        alignment.v_length = (
            alignments[line['seq_id']]['cdr3_start'] - alignment.seq_offset
        ) - gaps_in_seq
        alignment.j_length = j_length
        alignment.v_mutation_fraction = 1 - (
...
def add_sequences_from_sample(session, sample, sequences, props):
    logger.info(
        'Adding {} corrected sequences to sample {}'.format(
    ...
    for sequence in periodic_commit(session, sequences):
        ...
        try:
            ...
            if sequence['r_type'] == 'NoResult':
                add_sequences(
                    session, [alignment], sample, error_action='raise'
                )
...
def remove_duplicates(session, sample):
    ...
    seqs = (
        ...
        .filter(
            Sequence.locally_aligned.is_(True), Sequence.sample_id == sample.id
        )
    ...
    for seq in seqs:
        ...
        for other_seq in potential_collapse:
            if other_seq.seq_id == seq.seq_id or len(other_seq.sequence) != len(
                seq.sequence
            ):
                continue
