                    ...
                    self._session.add(
                        SequenceCollapse(
                            **{
                                ...
                                'collapse_to_subject_sample_id': larger[
                                    'sample_id'
                                ],
...
def run_collapse(session, args):
    mod_log.make_mod('collapse', session=session, commit=True, info=vars(args))
    subject_ids = []
    ...
    for subject in subjects:
        if (
            ...
            .filter(
                ...
                ~exists().where(SequenceCollapse.sample_id == Sample.id),
            )
            .first()
            is None
        ):
            logger.info(
                'Subject {} already collapsed.  Skipping.'.format(subject)
            )
        else:
            logger.info(
                'Resetting collapse info for subject {}'.format(subject)
            )
            samples = session.query(Sample).filter(Sample.subject_id == subject)
    ...
    logger.info(
        'Creating task queue to collapse {} subjects.'.format(len(subject_ids))
    )
