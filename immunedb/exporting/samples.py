...
def get_samples(session, for_update=False, sample_ids=None):
    meta = [
        s.key
        for s in session.query(SampleMetadata.key)
        .group_by(SampleMetadata.key)
        .order_by(SampleMetadata.key)
    ]
    ...
    clone_cnts = {
        ...
        .filter(~CloneStats.sample_id.is_(None))
    ...
    for sample in samples.order_by(Sample.name):
        logger.info(
            'Exporting sample {} (ID {})'.format(sample.name, sample.id)
        )
        row = {
            'id': sample.id,
            'name': sample.name,
        ...
        if not for_update:
            v_iden = (
                session.query(func.avg(CloneStats.avg_v_identity).label('avg'))
                .filter(CloneStats.sample_id == sample.id)
                .first()
            )
...
def write_samples(
    session, sample_ids=None, for_update=False, zipped=False, **kwargs
):
