...
mappings = {
    'airr': OrderedDict(
        (
            ...
            (
                'duplicate_count_in_subject',
                lambda s: s.collapse.copy_number_in_subject,
            ),
        )
    ),
    'changeo': OrderedDict(
        (
            ...
            (
                'V_CALL',
                lambda s: s.v_gene.replace(
                    '|', ',' + GeneName(s.v_gene).prefix
                ),
            ),
            (
                'J_CALL',
                lambda s: s.j_gene.replace(
...
class SequenceWriter(StreamingTSV):
    ...
    def __init__(self, format_name, metadata_fields):
        ...
        fields = list(self.mapping.keys()) + list(
            sorted([self.META_PREFIX + m for m in metadata_fields])
        )
...
def get_sequences(session, sample, fmt, clones_only, min_subject_copies):
    ...
    seqs = (
        ...
        .filter(Sequence.sample_id == sample.id)
    ...
    **kwargs,
    ...
    with ExportWriter(zipped=zipped) as fh:
        for sample in samples:
            ...
            fh.write(
                get_sequences(
                    session, sample, out_format, clones_only, min_subject_copies
                )
            )
