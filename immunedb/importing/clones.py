...
def generate_template(session, out_file):
    with open(out_file, 'w+') as fh:
        ...
        template_seqs = (
            ...
            .options(joinedload(Sequence.collapse))
...
def import_template(session, in_file, regen):
    ...
    with open(in_file) as fh:
        ...
        if 'ai' not in reader.fieldnames or 'clone_id' not in reader.fieldnames:
        ...
        for line in reader:
            ...
            clone_info['seqs'].append(
                {'ai': int(line['ai']), 'sample_id': int(line['sample_id'])}
            )
        session.commit()
